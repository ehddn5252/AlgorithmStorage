        ...
        if (direction == 0) {
            ...
            if(!unMarking) {
                for (int i = polledI - 1; i >= 0; --i) {
                    if (map[i][polledJ] == 1 || map[i][polledJ] == 2) return false;
                }
            }
            for (int i = polledI - 1; i >= 0; --i) {
            ...
            if(!unMarking) {
                for (int j = polledJ - 1; j >= 0; --j)
                    if (map[polledI][j] == 1 || map[polledI][j] == 2) return false;
            }
            for (int j = polledJ - 1; j >= 0; --j) map[polledI][j] = markingValue;
            ...
            for (int i = polledI + 1; i < map.length; ++i) map[i][polledJ] = markingValue;
        }
        return true;
    }
