...
public class BUK04_KIMDONGWOO_KIMSOOJIN0 {
    ...
    // 이거 홀의 위치를 각 공의 위치마다 다르게 구현해야 한다. 만약에 오른쪽에서 왼쪽으로 친다면 바로 들어가지 않고 위쪽 쿠션에 맞게 쳐야하고
...
//               power = Math.max(1f,(float) (hitList.get(0).power+)/6.12f);
                    power = 40f;
                    power = (float)50f;//(float) (hitList.get(0).power+1.27)*1.2f/6.62f;
